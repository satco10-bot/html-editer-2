...
## 수동 테스트 시나리오: Z-Order(레이어 앞/뒤 순서)

아래 4가지는 같은 준비 상태에서 각각 독립적으로 확인합니다.

- 준비
  1. `index.html`을 열고 아무 fixture나 불러옵니다.
  2. 겹쳐 보이게 요소 2개 이상(텍스트/박스/이미지 슬롯)을 선택해 둡니다.
  3. 레이어 패널에서 선택한 요소 이름을 확인합니다.

- 한 단계 앞으로 (`앞으로`)
  1. 대상 요소를 선택합니다.
  2. `앞으로` 버튼을 한 번 클릭합니다.
  3. 기대 결과: 선택 요소가 바로 위 레이어 1칸만 올라갑니다.
  4. 최상단 요소에서 다시 누르면 "이미 가장 앞 레이어입니다." 상태 메시지가 보입니다.

- 한 단계 뒤로 (`뒤로`)
  1. 대상 요소를 선택합니다.
  2. `뒤로` 버튼을 한 번 클릭합니다.
  3. 기대 결과: 선택 요소가 바로 아래 레이어 1칸만 내려갑니다.
  4. 최하단 요소에서 다시 누르면 "이미 가장 뒤 레이어입니다." 상태 메시지가 보입니다.

- 맨 앞으로 (`맨앞`)
  1. 대상 요소를 선택합니다.
  2. `맨앞` 버튼을 클릭합니다.
  3. 기대 결과: 선택 요소가 즉시 최상단 레이어로 이동합니다.

- 맨뒤로 (`맨뒤`)
  1. 대상 요소를 선택합니다.
  2. `맨뒤` 버튼을 클릭합니다.
  3. 기대 결과: 선택 요소가 즉시 최하단 레이어로 이동합니다.
